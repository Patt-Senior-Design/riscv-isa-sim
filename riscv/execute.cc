...
static void commit_log_print_value(rv_ostream_t& ostream, int width, const void *data)
{
  switch (width) {
    case 8:
      ostream.printf("0x%01" PRIx8, *(const uint8_t *)data);
      break;
    case 16:
      ostream.printf("0x%04" PRIx16, *(const uint16_t *)data);
      break;
    case 32:
      ostream.printf("0x%08" PRIx32, *(const uint32_t *)data);
      break;
    case 64:
      ostream.printf("0x%016" PRIx64, *(const uint64_t *)data);
      break;
    default:
      // max lengh of vector
      if (((width - 1) & width) == 0) {
        ...
        for (int idx = width / 64 - 1; idx >= 0; --idx) {
          ostream.printf("%016" PRIx64, arr[idx]);
        }
      } else {
        abort();
      }
...
static void commit_log_print_value(rv_ostream_t& ostream, int width, uint64_t val)
{
  commit_log_print_value(ostream, width, &val);
}
...
static void commit_log_print_insn(processor_t *p, reg_t pc, insn_t insn)
{
  ...
  rv_ostream_t line(line_buf);

  // print core id on all lines so it is easy to grep
  line.printf("core%4" PRId32 ": ", p->get_id());

  line.printf("%1d ", priv);
  commit_log_print_value(line, xlen, pc);
  line << " (";
  commit_log_print_value(line, insn.length() * 8, insn.bits());
  line << ")";
  bool show_vec = false;
  ...
  for (auto item : reg) {
    ...
    if (!show_vec && (is_vreg || is_vec)) {
        line.printf(" e%ld %s%ld l%ld",
                  p->VU.vsew,
                  p->VU.vflmul < 1 ? "mf" : "m",
                  p->VU.vflmul < 1 ? (reg_t)(1 / p->VU.vflmul)
                                   : (reg_t)p->VU.vflmul,
                  p->VU.vl);
        show_vec = true;
    }
    ...
    if (!is_vec) {
      if (prefix == 'c')
        line.printf(" c%d_%s ", rd, csr_name(rd));
      else
        line.printf(" %c%2d ", prefix, rd);
      if (is_vreg)
        commit_log_print_value(line, size, &p->VU.elt<uint8_t>(rd, 0));
      else
        commit_log_print_value(line, size, item.second.v);
    }
  }

  for (auto item : load) {
    line << " mem ";
    commit_log_print_value(line, xlen, std::get<0>(item));
  }
  ...
  for (auto item : store) {
    ...
    commit_log_print_value(line, xlen, std::get<0>(item));
    line << " ";
    commit_log_print_value(line, std::get<2>(item) << 3, std::get<1>(item));
  }
